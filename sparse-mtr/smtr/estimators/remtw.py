import numpy as np
from .. import utils
from .solvers import solver_remtw
...
class ReMTW(BaseEstimator):
    ...
    def __init__(self, M, alpha=1., beta=0., epsilon=None, gamma=None,
                 ...
                 gpu=True,  reweighting_steps=10, **kwargs):
        ...
        beta2: float.
            l1 norm regularization parameter on the barycenter.
        gamma: float.
...
if __name__ == "__main__":
    ...
    mtw_model = ReMTW(M=M, alpha=alpha, beta=beta, epsilon=epsilon,
                      gamma=gamma, stable=stable, tol_ot=1e-6, tol=1e-5,
                      warmstart=False, maxiter_ot=1000, maxiter=20000,
                      **callback_options)
    t = time()
    mtw_model.fit(X, Y)
    log = mtw_model.log_
