...
class MLL(BaseEstimator):
    ...
    """
    def __init__(self, alpha=None, positive=False, callback=False,
                 tol=1e-3, maxiter=1000, **kwargs):
    ...
    def get_params_grid(self, X, y, cv_size=20, eps=0.01):
        X, Y = self._pre_fit(X, y)
        n_samples = X.shape[1]
        xty = np.array([x.T.dot(yy) for (x, yy) in zip(X, Y)])
        alphamax = abs(xty).max() / n_samples
        scale = np.linspace(eps, 0.9, cv_size)
