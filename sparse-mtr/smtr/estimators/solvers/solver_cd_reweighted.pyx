    ...
    for i in range(maxiter):
        ...
        for j in range(n_features):
            ...
            if a > 0. or positive:
                ...
                theta[j] = fsign(tmp) * fmax(fabs(tmp) - weights[j] * sigma[0],
                                             0.) / Ls[j]
...
def cython_wrapper(int n_samples, int n_features, floating[::1, :] X,
                   ...
                   floating a, floating[:] weights, floating sigma0=0.,
                   floating tol=1e-4, int maxiter=10000,
                   bint positive=0):
    cdef:
        floating[:] sol
    with nogil:
        sol = cython_solver(n_samples, n_features, X, y, Ls,
                            marginals, theta, R, sigma, a, weights,
