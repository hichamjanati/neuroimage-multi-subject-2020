...
cdef floating proximal(floating theta, floating marginal,
                       floating a, floating b, floating threshold) nogil:
    ...
    cdef:
        ...
        floating delta
    z = theta - a - b
    if z * threshold + a * marginal <= 0:
        theta = 0.
    else:
        delta = sqrt((z + threshold) ** 2 + 4 * a * marginal)
        theta = (z - threshold + delta) / 2
    return theta
...
cdef floating cd_update(int n_samples, int n_features,
                        ...
                        int j,
                        floating sigma0=0.,
                        bint positive=0, floating threshold=1e-5,
...
cdef int cython_solver(int n_samples, int n_features,
                               ...
                               bint positive=0, floating threshold=1e-5) nogil:
...
def cython_wrapper(int n_samples, int n_features, floating[::1, :] X,
                   ...
                   floating a, floating[:] b, long[:] working_set,
                   floating sigma0=0.,
                   floating tol=1e-4, int maxiter=10000,
                   bint positive=0, floating threshold=1e-5):
    cdef:
        floating[:] sol
        int ws_size = len(working_set)
    with nogil:
        i = cython_solver(n_samples, n_features, X, y, Ls,
                            marginals, theta, R, sigma, a, b, working_set,
                            ws_size, sigma0, tol, maxiter, positive, threshold)
...
# @cython.wraparound(False)
# @cython.cdivision(True)
# def cython_wrapper_all(int n_tasks, int n_samples, int n_features,
#                        floating[:, :, :] X,
#                        floating[:, :] y, floating[:, :] Ls,
#                        floating[:, :] marginals,
#                        floating[:, :] theta, floating[:, :] R,
#                        floating[:] sigma,
#                        floating a, floating[:, :] b, long[:, :] working_set,
...
#         floating[:, :] sol
#         int ws_size = len(working_set)
#         long[:] niters_cd = np.zeros(n_tasks).astype(long)
#
#     if True:
#         for k in range(n_tasks):
#             i = cython_solver(n_samples, n_features, X[k], y[k], Ls[k],
#                               marginals[:, k], theta[:, k], R[k],
#                               sigma[k:],
#                               a, b[:, k], working_set[:, k],
#                               ws_size, sigma0, tol, maxiter, positive,
#                               threshold)
#             niters_cd[k] = i
#     return np.asarray(theta), np.asarray(R), np.asarray(sigma), niters_cd
