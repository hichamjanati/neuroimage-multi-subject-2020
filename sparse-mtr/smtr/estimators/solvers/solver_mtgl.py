...
def solver_mtgl(X, y, coefs0=None, R=None, alpha=1., beta=1.,
    ...
    if positive:
        theta1, R, loss = solver_numba_positive(X, y, coefs01, R, Ls, alpha,
                                                maxiter, tol, verbose,
                                                computeobj)
    else:
        theta1, R, loss = solver_numba(X, y, coefs01, R, Ls, alpha,
...
@jit(output_type(float64[::1, :, :], float64[::1, :], float64[::1, :],
                 float64[::1, :], float64[:], float64, int64,
                 float64, boolean, boolean),
     nopython=True, cache=True)
def solver_numba_positive(X, y, theta, R, Ls, alpha, maxiter, tol,
                          verbose, computeobj):
...
@jit(output_type(float64[::1, :, :], float64[::1, :], float64[::1, :],
                 ...
                 float64, boolean, boolean),
     nopython=True, cache=True)
def solver_numba(X, y, theta, R, Ls, alpha, maxiter, tol,
                 verbose, computeobj):
    """Perform GFB with BCD to solve Multi-task group lasso."""
    n_tasks = len(X)
    n_samples, n_features = X[0].shape
