...
from .solver_cd_reweighted import cython_wrapper
try:
    import cupy as cp
...
def solver_remtw(X, Y, coefs01=None, coefs02=None, R=None, b1=None, b2=None,
    ...
    with Parallel(n_jobs=n_jobs, backend="threading") as pll:
        if alpha == 0.:
            ...
            theta, R, sigmas = update_coefs(pll, X, Y, Ls, marginals1,
                                            ...
                                            positive=positive,
                                            cython=cython,
                                            steps=reweighting_steps)
            obj = 0.5 * (R ** 2).sum(axis=1).dot(1 / sigmas) / n_samples
            obj += beta * (abs(theta) ** 0.5).sum() + 0.5 * sigmas.sum()
            ...
            return (theta1, theta2, R, thetabar1, thetabar2, log, sigmas, b1,
                    b2, marginals1, marginals2)
        for i in range(maxiter):
            ...
            if alpha:
                ...
                if not positive and theta2.max(0).all():
...
def update_coefs(pll, X, y, Ls, marginals, sigmas, a, b, sigma0,
                 coefs0=None, R=None, maxiter=20000, tol=1e-4, cython=True,
                 positive=False, steps=1):
    """BCD in numba."""
    n_tasks, n_samples, n_features = X.shape
    ...
    weights = b * np.ones((n_features, n_tasks))
    for ii in range(steps):
        if cython:
            dell = delayed(cython_wrapper)
            it = (dell(n_samples, n_features, np.asfortranarray(X[k].copy()),
                       y[k], Ls[k], marginals[k], coefs0[:, k], R[k].copy(),
                       sigmas[k:k + 1], a, weights[:, k], sigma0, tol, maxiter,
                       positive)
                  for k in range(n_tasks))
        else:
            dell = delayed(nb_solver_wrapper)
            it = (dell(X[k], y[k], Ls[k], marginals[k],
                       coefs0[:, k], R[k], sigmas[k], a, weights[:, k], sigma0,
                       tol, maxiter, positive) for k in range(n_tasks))
        output = pll(it)

        thetas, R, sigmas = list(zip(*output))
        thetas = np.stack(thetas, axis=1)
        weights = b * 0.5 / (abs(thetas) ** 0.5 + 1e-10)
        R = np.stack(R, axis=0)
...
def nb_solver(X, y, Ls, marginals, theta, R, sigma, a, weights, sigma0,
        ...
        for j in range(n_features):
            ...
            if a or positive:
                theta[j] = proximal(tmp, marginals[j] * Ls[j], a,
                                    weights[j] * sigma) / Ls[j]
            else:
                theta[j] = fsign(tmp) * max(abs(tmp) -
                                            weights[j] * sigma, 0) / Ls[j]
