...
import warnings
import numpy as np
from .utils import auc_prc, get_unsigned, residual
from .exceptions import NotFittedError
from .optimaltransport import otfunctions
...
class BaseEstimator(object):
    ...
    def score_coefs(self, coefs_true,
        ...
        mse = ((coefs_true - self.coefs_) ** 2).mean(0)
        mse = np.sqrt(mse).mean()
        i = 0
        if wyy0:
            w = 0.
        else:
        ...
        if compute_ot:
            otabs = self.ot_score(abs(coefs_true), abs(self.coefs_), M,
                                  ...
                                  wyy0=w, **kwargs)
        i = max(i, 1)
        metrics = dict(auc=auc / i, mse=-mse, ot=-ot / i, aucabs=aucabs,
                       otabs=-otabs)
        return metrics

    def get_gof(self, X, y_true, coefs):
        y_pred = np.array([x.dot(c) for x, c in zip(X, coefs.T)])
        numerator = ((y_true - y_pred) ** 2).sum(axis=1)
        denominator = (y_true - np.average(
            y_true, axis=1) ** 2).sum(axis=1)
